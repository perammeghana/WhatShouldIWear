...
def get_timezone(lat, lon, apiKey):
    # Get the current timestamp in seconds
    timestamp = datetime.now().timestamp()

    # Send an HTTP GET request to the Google Time Zone API
    url = f"https://maps.googleapis.com/maps/api/timezone/json?location={lat},{lon}&timestamp={timestamp}&key={apiKey}"
    response = requests.get(url)

    # Check if the response was successful
    if response.status_code != 200:
        return None, "Error: Unable to get timezone information"

    # Parse the JSON response to extract the timezone information
    data = response.json()
    if "timeZoneId" not in data:
        return None, "Error: Invalid API key or location coordinates"

    timezone_id = data["timeZoneId"]
    # timezone_name = timezone_id.replace("_", " ")

    # Convert the timezone offset to a timezone abbreviation like "EDT"
    timezone = pytz.timezone(timezone_id)
    timezone_abbreviation = timezone.tzname(datetime.now())
    return str(timezone_abbreviation), str(timezone)
...
def getDataForLocations(weeklyOrhourly, allOrsingle):
    ...
    if (str(allOrsingle) == 'all' and str(weeklyOrhourly) == 'weekly'):
        for row in df_locations:
            timezone, timezoneFormat = get_timezone(row[1], row[2], api_key)
    ...
    # connection.close()
    # delete cron logs older than two weeks
    cursor2 = connection.cursor()
    sqlLog2 = "DELETE FROM cronlogs WHERE CronStartTime < NOW() - INTERVAL 2 WEEK;"
    try:
        # Execute the SQL command
        cursor2.execute(sqlLog2)
        connection.commit()
    except pymysql.connect.Error as error:
        print("Failed to delete cronlogs older than two weeks {}".format(error))
    connection.close()
